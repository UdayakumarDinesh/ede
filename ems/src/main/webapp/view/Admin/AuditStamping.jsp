...
<div class="nav navbar auditnavbar" style="background-color: white;margin-top: -18px">
		...
		<div class="row ">
			<input type="hidden" name="${_csrf.parameterName}"	value="${_csrf.token}" /> 
				   ...
				   <label style="margin-left: 70px; margin-right: 10px; color: black;">User Name: <span class="mandatory" style="color: red;">*</span></label>
					...
					<select class="form-control form-control select2" name="loginid" style="margin-left: 12px;" required="required" id="username" >
			    	
			    	<%if("A".equalsIgnoreCase(LoginType)){ %>
			    	
			    		<option value="0" <% if(loginid.toString().equalsIgnoreCase("0")){%> selected<% ListName ="All"; }%> >All</option>
			    		
			    	<%} %>
				    <%for(Object[] obj:emplist){%>
				    	
						<option value="<%=obj[0]%>"<% if(obj[0].toString().equalsIgnoreCase(loginid)){%> selected<% ListName =obj[3].toString(); }%> ><%=obj[3]%></option>
					<%} %>
					</select>
					...
					<input type="text" class="form-control input-sm fromdate" style="width: 140px;" readonly="readonly" value=""   id="fromdate" name="fromdate"  required="required"  > 
				
				
					<label style="margin-left: 10px; margin-right: 20px; color: black;">To Date: <span class="mandatory" style="color: red;">*</span></label>
					
					
					<input type="text" class="form-control input-sm todate" style="width: 140px;" readonly="readonly" value=""  id="todate" name="todate"  required="required" > 			
 ...
 <div class="row">
	<div class="col-md-9">
		...
		<h6><%if(ListName != null){%>Details of <b><%=ListName.toUpperCase()%></b><%}else{%> Details of <b><%=Username.toUpperCase()%></b> <%}%> from <span class="datefont"><%=DateTimeFormatUtil.SqlToRegularDate(Fromdate)%></span> to <span class="datefont"><%=DateTimeFormatUtil.SqlToRegularDate(Todate) %></span></h6>
		</div>
	</div>	
</div> 
