...
<%
	...
	List<Object[]> CHSSEligibleFamList = (List<Object[]>)request.getAttribute("CHSSEligibleFamList");
	...
	ArrayList<String> addedfammemidlist = new ArrayList<String>(); 
	
	ExcMemberDetails.stream().forEach(obj -> addedfammemidlist.add(obj[0].toString()));
%>
<body >
			...
			<div class="col-md-5">
				...
				<div class="col-md-7">
					<ol class="breadcrumb ">
						...
						<%} %>
						<li class="breadcrumb-item active " aria-current="page">CHSS FORM -1</li>
					</ol>
				</div>
		</div>
	</div>
		...
		<!-- 	<div align="right">
			...
			<div align="left">
				...
				<table>
					...
					<% int count=0;
					for(Object[] member : CHSSEligibleFamList){
						if(!addedfammemidlist.contains(member[0].toString())){%>
					<tr>
						<td><%=++count%></td>
						<td><%=member[1] %></td>
						<td><%=member[7] %></td>
						<td style="text-align: center;"><%=DateTimeFormatUtil.SqlToRegularDate(member[3].toString()) %></td>
						<td><%if(member[10]!=null){ %> <%=member[10] %> <%}else{ %> - <%} %></td>
						<td><%if(member[11]!=null){ %> <%=member[11] %> <%}else{ %> 0 <%} %></td>
						<%if((status.equalsIgnoreCase("C") || status.equalsIgnoreCase("R"))){ %>
						<td style="text-align: center;">	
							<form action="DepExcusionFormMemAdd.htm" method="POST" enctype="multipart/form-data" autocomplete="off" >
								<button  type="submit" class="btn btn-sm "  onclick="hideEdit('<%=member[0] %>')" title="Exclude"> 
									<i class="fa-solid fa-xmark"  style="color: red;"></i>
								</button>
								
								<input type="hidden" name="familydetailsid" value="<%=member[0]%>"/>
								<input type="hidden" name="Decformid" value="<%=formid%>"/>
								<input type="hidden" name="formid" value="<%=formid%>"/>
								<input type="hidden" name="mem-exc-date" value="<%=DateTimeFormatUtil.SqlToRegularDate(LocalDate.now().toString())%>"/>
...
<!-- -------------------------------- Exclude --------------------------------------  -->
			...
			<div align="left">
				...
				<table>
					...
					<%  count=0;
					boolean listzero = false;
					
					for(int i=0;i<ExcMemberDetails.size() ;i++)
					{
						if(ExcMemberDetails.get(i)[19].toString().equalsIgnoreCase("E")){
						%>
						...
						<tr id="show-view-<%=ExcMemberDetails.get(i)[0] %>"   >
							...
							<td style="text-align: center;">
								<%if(ExcMemberDetails.get(i)[18]!=null){ %>
									<button type="submit" class="btn btn-sm" name="formmemberid" value="<%=ExcMemberDetails.get(i)[15] %>"  formaction="FamIncExcAttachDownload.htm" formtarget="_blank"  data-toggle="tooltip" data-placement="top" title="Download">
							...
							<%if(status.equalsIgnoreCase("C") || status.equalsIgnoreCase("R")){ %>
								<td>
									<button type="submit" class="btn btn-sm" formnovalidate="formnovalidate" name="formmemberid" value="<%=ExcMemberDetails.get(i)[15] %>" formaction="ExcFormRemoveMember.htm"  onclick="return confirm('Are You Sure to Remove ?');"  title="Remove">
...
<!-- --------------------------------exclude----------------------------------- -->							
				...
				<table>
					...
					{
						if(FwdMemberDetails.get(i)[20].toString().equalsIgnoreCase("I")){

						excludeedlist.add(FwdMemberDetails.get(i)[0].toString());
						++count1;
						if(FwdMemberDetails.get(i)[19].toString().equals("0"))
						{ %>
						
							<tr id="show-edit-<%=FwdMemberDetails.get(i)[0] %>" style="display: none;" >
								<td style="text-align: center;" ><form action="DepMemEditSubmit.htm" method="POST" enctype="multipart/form-data" autocomplete="off" id="edit-form-<%=FwdMemberDetails.get(i)[0] %>" ><%=count1%></form></td>
								<td>
								...
								<td>	
									...
									<input form="edit-form-<%=FwdMemberDetails.get(i)[0] %>" type="hidden" name="incformid" value="<%=formid%>"/>
									<input form="edit-form-<%=FwdMemberDetails.get(i)[0] %>" type="hidden" name="Decformid" value="<%=formid%>"/>
								</td>
							</tr>
						<%}else { %>
					...
					<tr>
						...
						<tr  >
							...
							<td>	
								...
								<input form="add-form-0" type="hidden" name="Decformid" value="<%=formid%>"/>
							</td>
						...
						<tr>
							...
							<td>	
								...
								<input form ="add-form" type="hidden" name="Decformid" value="<%=formid%>"/>
							</td>
						</tr>
				...
				<p style="text-indent: 50px;">
					...
					I have read the instructions on the reverse side very carefully and have understood their meaning. 
					I certify that the persons mentioned above fulfil the conditions prescribed and that they are 
					eligible for registration under CHSS
				</p>
				<p style="text-indent: 50px;">
					
					I hereby undertake to declare at the beginning of each calendar year and as soon as necessary thereafter
					 about the eligibility or otherwise to the CHSS benefits of myself and my parents / family members whose names are 
					 mentioned above. It shall be my responsibility to notify the Unit when any person referred to above becomes 
					ineligible to the CHSS benefits. I realise that the onus of proving eligibility of the members 
					mentioned above to the benefits of the scheme rests on me.
				</p>
				
				<div align="center"><b style="text-decoration: underline; font-size: 20px;" >Certificate in respect of dependant</b></div>
				<p>
				I certify that the family members whose names are mentioned above are mainly dependant on and residing with me.
				</p>
				<h1 class="break"></h1>
				<br><br>
				<div align="left">
